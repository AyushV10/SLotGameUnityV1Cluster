...
namespace BE {
	...
	public class Line {
		...
		public Line(int s0, int s1, int s2, int s3, int s4, int s5,string strColor) {
			Slots[0] = s0;
			Slots[1] = s1;
			Slots[2] = s2;
			Slots[3] = s3;
	...
	public class SlotGame : MonoBehaviour {
		...
		public void AnimateSymbol(int repeatcount)
		{
			string[] coordinates = SceneSlotGame.slotGameData.Entries[spinCount].Coordinates;
			...
			foreach (string coordStr in coordinates)
			{
				...
				int y = (Reels.Count-1)-int.Parse(parts[1]);
				// Print or process the coordinates
				//Debug.Log("X:" + x + "-------" + y + "= y");
				if (Reels[x].Symbols[y] != null) {
					//Transform symbolTransform = Reels[x].Symbols[y].transform;
					//Debug.Log("X:" + x + "-------" + y + "= y" + Reels[x].Symbols[y].name);
					//float newScale = Mathf.PingPong(Time.time * scaleSpeed, maxScale);
					//// Apply the new scale to the symbol
					//symbolTransform.localScale = new Vector3(newScale, newScale, newScale);
					StartCoroutine(ScaleSymbol(Reels[x].Symbols[y].transform, maxScale, animationDuration, repeatcount));
				}
			}
		}
		...
		private IEnumerator ScaleSymbol(Transform symbolTransform , float maxScale , float duration , int repeatcount) {
			Vector3 originalScale = symbolTransform.localScale;
			for (int i = 0; i < repeatcount; i++) { 
				float timer = 0f;
				BEAudioManager.SoundPlay(1);
				while (timer < duration) {
					float newScale = Mathf.Lerp(originalScale.x, maxScale, timer / duration);
					symbolTransform.localScale = new Vector3(newScale, newScale, newScale);
					timer += Time.deltaTime;
					yield return null;
				}
				symbolTransform.localScale = originalScale;
				yield return new WaitForSeconds(0.5f);
			}
		}
		public void SymbolInsert(int ID) 	{ if(Application.isPlaying) return; 	Symbols.Insert(ID, new Symbol("Noname", SymbolType.Normal, 0, "0,0,0,0,0,0")); }
		...
		public void LineInsert(int ID) 		{ if(Application.isPlaying) return; 	Lines.Insert(ID, new Line(0,0,0,0,0,0,"000000")); }
		...
		public SlotReturnCode Spin() {
			Debug.Log("Result="+SceneSlotGame.slotGameData.Entries[0].FG_Triggred);
			GC.Collect();
		...
		public List<int> GetDeck(int cnt)
		{
			List<int> result = new List<int>();
			int length = SceneSlotGame.slotGameData.Entries[cnt].ReelWindowRows.Length;
			string[] newArr = new string[length];
			for (int i = length - 1; i >= 0; i--)
			{
				newArr[length - 1 - i] = SceneSlotGame.slotGameData.Entries[cnt].ReelWindowRows[i];
			}
			foreach (string row in newArr)
			{
				string[] symbols = row.Split(' '); // Split the row by space
				//Array.Reverse(symbols);
				foreach (string symbol in symbols)
				{
					if (int.TryParse(symbol, out int SymbolIdx)) // Parse each symbol
					{
		...
		// Transpose function to switch rows and columns
		private string[][] Transpose(string[] rows)
		{
			int rowCount = rows.Length;
			int colCount = rows[0].Split(' ').Length;
			string[][] transposedRows = new string[colCount][];
			for (int i = 0; i < colCount; i++)
			{
				transposedRows[i] = new string[rowCount];
				for (int j = 0; j < rowCount; j++)
				{
					transposedRows[i][j] = rows[j].Split(' ')[i];
				}
			}
			return transposedRows;
		}
		public void ApplyResult(int count)
		{
			// Create Result Symbols
			Deck = GetDeck(count);

			// Start Reels spin
			for (int x = Reels.Count-1; x >=0; --x)
			{
				// Make array of final value for each reel
				int[] value = new int[RowCount];
				// Assign values from Deck to reels from left to right
				for (int y = 0; y < RowCount; ++y)
					value[y] = Deck[x + Reels.Count * y];
			...
			// Bonus, Scatter Spin Effect
			{
				...
				for (int x = 0; x < Reels.Count; ++x)
				{
					...
					for (int y = 0; y < RowCount; ++y)
					{
						SymbolType eType = GetSymbol(Deck[x + Reels.Count * y]).type;
		...
		public void Simulation() {
			...
			for(int c=0 ; c < SimulationCount ; ++c) {
				...
				// get result
				List<int> iDeck = GetDeck(spinCount);
